...
var pendingecho = 0;
window.useaudio = true;
var client, socket;
import GrayScalePipeline from '../assets/pipelines/GrayScale.js';

function curTime() {
    var dt = new Date();
    var ret = '[' + ('0' + dt.getHours()).slice(-2) + ':' + ('0' + dt.getMinutes()).slice(-2) + ':';
    ret = ret + ('0' + dt.getSeconds()).slice(-2) + ']';
    return ret;
}

var gs, timeline, tablesurface, tablesurfacei, tablesurfaceColors;
var tablesurfaceColors = [0, 0, 0, 0];
...
var cardsets = {"cardset-0": {ocards: 'assets/ocards.png',
	cardback: 'assets/cardback.png',
	cardsinfo: 'assets/cardsinfo.json'},
	"cardset-1": {ocards: 'assets/ocards_alt1.png',
		cardback: 'assets/cardback_alt1.png',
		cardsinfo: 'assets/cardsinfo_alt1.json'}};
var cardsetsel = 'cardset-0';

scene.preload = function () {
	/*
	 * Following Progress bar initializations from
	 * Scott Westlover's article "Creating a Preloading Screen"
	...
	this.load.on('complete', function () {
		...
		percentText.destroy();
		assetText.destroy();
		this.removeListener('progress');
		this.removeListener('fileprogress');
		this.removeListener('complete');
	});
    this.load.image('pointerselect', 'assets/slpo.png');
    this.load.image('crowncork', 'assets/crowncork.png');
    this.cache.json.remove('cardsinfo');
    this.textures.remove('ocards');
    this.textures.remove('cardback');
    this.load.image('cardback', cardsets[cardsetsel].cardback);
    this.load.spritesheet('ocards', cardsets[cardsetsel].ocards,
			  { frameWidth: 206,
			    frameHeight: 365,
			    startFrame: 2, endFrame: 53 });
    this.load.json('cardsinfo', cardsets[cardsetsel].cardsinfo);
    preloadAudio(this);
};

function createTablesurface() {
	tablesurface = scene.add.graphics();
	let somcols = getTableSurfaceColors();
	tablesurface.fillGradientStyle(somcols[0], somcols[1], 
		somcols[2], somcols[3], 1);
	tablesurface.fillRect(0, 0, scnwidth, scnheight);
	tablesurfacei = 1;
}
...
scene.create = function() {
   ...
   // addAudios();
	...
	splay(scene, "pop", "startwc", {volume: 0.05});
	...
	onLoop: function () {
		...
		splay(scene, "accordion", "startwc", {volume: 0.03});
	},
	onStart: function () {
	    let cardcaption = scene.data.get('cardcaption');
...
scene.update = function () {
     ...
     * Selecting a tabla
     */
	if ((gs.phase == "cantando") && ((tablesurfacei % 120)  == 0)) {
		let ncolos = getTableSurfaceColors(tablesurfacei);
		tablesurface.fillGradientStyle(ncolos[0], ncolos[1],
			ncolos[2], ncolos[3], 1);
		tablesurface.fillRect(0, 0, scnwidth, scnheight);
	}
	tablesurfacei++;
	...
	if (this.data.has("tablas")) {
	    ...
	    splay(scene, "bees", "startwc", {loop:true});
	} else {
	...
	nallcards.list[j].setX(nallcardslocs[j][0]);
      ...
      if (singlemode) {
	gs.randseed = CryptoJS.MD5(JSON.stringify("singlemode: " + new Date().getTime()));
	shuffle(gs.cards, gs.randseed);
	console.log("after shuffle");
	splay(scene, "bees", "stop");
	splay(scene, "mariachi", "startwc", {loop:true, volume: 0.05, delay: 10});
	gs.phase = "cantando";
...
var gs = { // game state
   ...
   // at: 30000,
	...
	case "shuffle":
		...
		if (msge[0] == "CS") {
		    ...
		    splay(scene, "bees", "stop");
			splay(scene, "mariachi", {loop: true, volume: 0.05, delay: 10});
		    this.phase = "cantando";
		    jQuery('#status').prepend(curTime() + ' Cantando<br />');
   ...
   noPendingEcho: async function () {
	let realphasepause = this.phasepause;
	this.phasepause = true;

	while (pendingecho > 0) {
		/* check back every second until no pending echo */
		await new Promise((resolve, reject) => setTimeout(null, 1000));
	}
	this.phasepause = realphasepause;
   },
 ...
 */    
	...
	this.phase = "end";
	splay(scene, "any", "stopall");
	splay(scene, "ganymede", "startwc", {loop: true});
    },
   
    startGame: async function () {
	var j = 0;
	jQuery('#status').prepend(curTime() + ' Waiting for game announcements.<br />');
 	splay(scene, "mariachi", "startwc", {volume: 0.02});
	splay(scene, "vendedora", "startwc", {loop: true});
 	let promise = new Promise((resolve, reject) => {
	...
	if (this.phase == "start_wait_an") {
		...
		await gs.noPendingEcho().then(() => this.sndMsg("!! HS " + this.gid));
		let promise = new Promise((resolve, reject) => {
	    		setTimeout(() => resolve('done!'), this.tslw)
			});
		let result = await promise;
...
jQuery(document).ready(function ()  {
	jQuery("#uselocalsrv").prop("checked", false);
	jQuery("#singlemode").prop("checked", false);
	jQuery("#ckuseaudio").prop("checked", true);
	jQuery(".dropdown-item").on('click', function (e) {
		cardsetsel = e.target.id;
		jQuery("#currentcardset").text(jQuery('#' + cardsetsel).text());
	});
	jQuery("#singlemode").on('click', function (e) {
    		if (jQuery("#singlemode").is(":checked")) {
      			LocalServer = false;
      			jQuery("#uselocalsrv").prop("checked", false);
      			jQuery("#uselocalsrv").attr("disabled", true);
      			singlemode = true;
      			jQuery("#joingame").html("Start");
		} else { 
			singlemode = false; 
			jQuery("#uselocalsrv").removeAttr("disabled");
			jQuery("#joingame").html("Login");
    		}
	});
	jQuery("#ckuseaudio").on('click', function (e) {
		if (jQuery("#ckuseaudio").is(":checked")) {
			window.useaudio = true;
		} else {
		    	splay("any", "stopall");
			window.useaudio = false;
		}
	});
	gs.showScoreTable();
		...
		if (buenatime) {
			...
			if (gs.isHost && gbuenares && !singlemode) {
				...
				scene.scene.start("winscene");
                  	...
                  	if (singlemode) {
				if (gbuenares) {
					jQuery('#status').prepend(curTime() + ' You win.<br />');
		    			splay(scene, "any", "stopall");
                    			scene.scene.stop();
                    			scene.scene.start("winscene");
				} else {
					jQuery('#status').prepend(curTime() + ' not a real buena.<br />');
				}
			}
		}
	...
	jQuery("#joingame").on('click', function (e) {
		...
		if (singlemode) {
			...
			gs.init();
			splay(scene, "anys", "stopall");
        		gs.phase = "table_selects";
      		} else if (!LocalServer && !client.getAccessToken()) {
			...
			client.once('sync', function(state, prevState, res) {
					...
					gs.startGame();
					jQuery("#joingame").prop("disabled", true);
		 			client.on("Room.localEchoUpdated", function(event, room, oEid, OStat) {
			 			pendingecho--;
		 			});
...
function getTableSurfaceColors(i) {
	/*
	 * Basically rotating colors here
	 */
	
	for (let n = 0; n < 4; n++) {
		let rancol = new Phaser.Display.Color.RandomRGB();
		tablesurfaceColors[n] = Phaser.Display.Color.GetColor(rancol.r, rancol.g, rancol.b);
	}
	return tablesurfaceColors;
}
