...
//	    this.input.setDraggable(tablas[i]);
	...
	for (var j = 0, k = -1; j < gs.tablas[i].length; j++) {
	    ...
	    // let sprite = this.add.sprite(sx, sy, 'card' + gs.tablas[i][j]);
	    let sprite = this.add.sprite(sx, sy, 'ocards', gs.tablas[i][j] - 1);
	    //sprite.setDisplaySize(gs.dims.tcard.w, gs.dims.tcard.h).setTintFill(0x0fff0f + (i * 200));
...
//	console.log("cards played = ", gs.cards_played);
//	console.log("includes(%s)", cardn);
    ...
    /*
    var text = this.add.text(10, 200, 'Move the mouse', { font: '16px Courier', fill: '#ffffff' });
    this.data.set('debugtext', text);
    */
...
/*
	    if (cardcaption.innerHTML == newcap)
		timeline.pause();
	    else
...
*/
	...
	duration: gs.cdtd,
	onComplete: function () {
//	    cardback.setTexture('card' + gs.current_card);
...
scene.update = function () {
    ...
    var text = this.data.get('debugtext');
    text.setText([
        'x: ' + pointer.x,
        'y: ' + pointer.y,
...
//			el.list[i].setData({occupied: false});
		    }
		  el.setScale(1.5);
		} else {
		    el.destroy();
		...
		sx -= (config.width / 2);
		sy -= (config.height / 2);
//		let sprite = this.add.sprite(sx, sy, 'card' + (j + 1));
		let sprite = this.add.sprite(sx, sy, 'ocards', j);
		//sprite.setDisplaySize(gs.dims.tcard.w, gs.dims.tcard.h).setTintFill(0x0fff0f + (j * 100));
		sprite.setDisplaySize(gs.dims.tcard.w, gs.dims.tcard.h);
		allcards.add(sprite);
		cardlocs.push([sx, sy]);
	    }
...
//	    let buenabtn = this.data.get('buenabtn');
	    cardback.visible = true;
	    pcardback.visible = true;
	    scrowncork.visible = true;
//	    buenabtn.visible = true;
 ...
 */
	...
	return this.tablasel.find(function (el) {
	    ...
	    if (el[0] == claimer) {
		var possible = true;
		//return gs.isWinning(parseInt(el[1]));
...
//			alert("isWinig returned true for " + claimer);
			return true;
		    }
		} else {
...
//		    if (gs.isHost)
//			console.log("Buena not possible");
		    return false;
		}
	    }
	});
